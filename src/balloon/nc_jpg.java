...
//        File file = new File("/Users/xupengwei/hehe/" + picType);
        ...
        try {
            BufferedReader in = new BufferedReader(new FileReader("D:\\learn\\qq\\qq (" + no + ").nc"));
            //关于文件改名，可以全选文件同时改名
            //注意qq与”（“之间的空格
//            in.close();
...
//                    g2.drawString("绘图",ix1,iy1); //向图片上写字符串
            ...
            ImageIO.write(bi, "JPEG", new FileOutputStream("D:\\learn\\p\\balloon(" + no + ").jpg"));//保存图片 JPEG表示保存格式
            System.out.println("第"+no+"张图片已完成");
        } catch (IOException e) {
